...
## 5. Wire into the hosts

**Web API host** (`src/Hosts/Sergin.MeterMinder.Hosts.WebApi.All/`):
- csproj — add `<ProjectReference Include="..\..\Modules\<Module>\Sergin.<Module>\Sergin.<Module>.csproj" />`.
- `Program.cs` — add `using Sergin.<Module>;` and one element to the modules collection: `IReadOnlyCollection<ISerginModule> modules = [new MeterMinderModule(), new UserAccessModule(), new <Module>Module()];` — nothing else; the bootstrap loops handle MediatR, DI, migrations, and endpoint mapping.

**Blazor UI host** (`src/Hosts/Sergin.MeterMinder.Hosts.WebUi.All/`) — only if the module has a UI:
- `Program.cs` — same one-element addition to the modules collection. `AddSerginWebUi`/`UseSerginWebUiAsync<App>` pick up the new `ISerginWebUiModule` automatically: its `UiAssembly` joins `AddAdditionalAssemblies`, its `NavItems` join the shared nav menu.
- csproj — add **two** `ProjectReference`s, not one: the composition root *and* `Sergin.<Module>.Presentation.Blazor` directly. The second looks redundant and is not. Static web assets (`_content/...`) propagate only through projects importing `Microsoft.NET.Sdk.StaticWebAssets`; the composition root is a plain `Microsoft.NET.Sdk`, so the chain host → composition root → RCL breaks silently at the middle hop (`ResolveReferencedProjectsStaticWebAssetsConfiguration` probes with `SkipNonexistentTargets="true"`, so nothing warns — the CSS just 404s at runtime and the UI renders unstyled). The existing csproj carries a comment saying exactly this; add the new reference next to it.
- The UI host is Development-only and has no authentication: every request runs as the user configured under `Sergin:DevUser` in its `appsettings.json`. If the new module's slices carry `[RequiredPermissions]`, **add those permission strings to that `Permissions` array** or the pages will render a Forbidden problem panel. An invalid entry there fails startup by design, naming the exact key and value.
