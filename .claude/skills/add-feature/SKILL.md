...
**List query** (a paged `GetList` slice) is the same shape with three specifics. The request record inherits the shared base `ListQuery` and implements `IListQuery<Get<Aggregate>ListItem>`, declared with an explicit constructor rather than positional parameters — copy `GetDeviceListQueryCommand.cs`:

```csharp
[RequiredPermissions("permission.<schema>.<resource>.read")]
public sealed record Get<Aggregate>ListQueryCommand : ListQuery, IListQuery<Get<Aggregate>ListItem>
{
