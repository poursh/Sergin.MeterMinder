...
| `Models/New<Aggregate>FormModel.cs` | — | `public sealed class`, `System.ComponentModel.DataAnnotations` attributes; `[StringLength(Domain.<Aggregate>s.<ValueObject>.MaxLength, MinimumLength = 1)]` reads the same constant the validator does (qualified — the model's own property of that name shadows the type inside the attribute) |
