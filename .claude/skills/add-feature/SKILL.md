...
3b. `src/Modules/<Module>/Sergin.<Module>.Application/<Aggregate>/Commands/<Feature>/<Feature>CommandValidator.cs` — every write command gets one: `internal sealed class <Feature>CommandValidator : AbstractValidator<<Feature>Command>` (`using FluentValidation;` per file, not in `GlobalUsings.cs`), rules in the constructor. Commands carry value objects, so target `.Value` and restore the command property name: `RuleFor(x => x.UserName.Value).NotEmpty().MaximumLength(UserName.MaxLength).OverridePropertyName(nameof(CreateUserCommand.UserName));` — the property name becomes `Error.Code`. An optional value object: `RuleFor(x => x.Address!.Value)....When(x => x.Address is not null)`. Put a length limit as `public const int MaxLength` on the value object in `.Domain` and read it from here and from the form model (step below), never as a literal in two places. Nothing to register — `AddSerginCore` scans `ApplicationAssembly` for `IValidator<>` implementations. Reference: `Users/Commands/Create/CreateUserCommandValidator.cs`, `Devices/Commands/Create/CreateDeviceCommandValidator.cs`.
   **Repository-backed rules** (`using Sergin.SharedKernel.Application.Validations;`) go in the same validator, after the shape rules. Inject the repositories through the validator's constructor — `CreateDeviceCommandValidator(IDeviceRepository devices, IManufacturerRepository manufacturers)` — which is fine because the validator is scoped and lives in the request scope with the `DbContext`. Two rules, each targeting the **wrapper** (so `Error.Code` is already the property name; no `OverridePropertyName`) and each behind a `.When` on its own shape rule so no query runs for a value already refused:
   - The command references another aggregate → `RuleFor(x => x.ManufacturerId).MustExistIn(manufacturers).When(x => x.ManufacturerId.Value != Guid.Empty);` (message `'Manufacturer Id' must refer to an existing Manufacturer.`).
   ...
   Both are advisory; the FK / unique index is the guarantee, and a race still throws `DbUpdateException` — don't try to catch it in the handler. A find-or-create command (`ProvisionExternalUser`) gets no uniqueness rule, since the value is legitimately taken by the row the handler is about to find.
